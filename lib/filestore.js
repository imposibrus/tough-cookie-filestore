...
FileCookieStore.prototype.getAllCookies = function(cb) {
    ...
    try {
        for(var domain in this.idx) {
            if(this.idx.hasOwnProperty(domain)) {
                for(var path in this.idx[domain]) {
                    if(this.idx[domain].hasOwnProperty(path)) {
                        for(var cookieKey in this.idx[domain][path]) {
                            if(this.idx[domain][path].hasOwnProperty(cookieKey)) {
                                results.push(this.idx[domain][path][cookieKey]);
                            }
                        }
                    }
                }
